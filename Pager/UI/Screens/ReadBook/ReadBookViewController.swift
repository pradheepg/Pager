...
class SettingsViewController: UIViewController {
    ...
    private let navigationLabel = SettingsLabel(text: "Page Turn")
    private lazy var navigationSegment: GenericSegmentedTile = {
        let items = [
            SegmentItem(title: "Slide", iconName: "text.page.fill"),
            SegmentItem(title: "Page ture", iconName: "book.pages")
        ]
        let segment = GenericSegmentedTile(items: items, defaultIndex: 0)
        segment.onSelectionChanged = { [weak self] index in
    ...
    private func setupLayout() {
        ...
        mainContainerStack.setCustomSpacing(12, after: themeLabel)
        
        
        mainContainerStack.addArrangedSubview(navigationLabel)
        mainContainerStack.addArrangedSubview(navigationSegment)
        
        mainContainerStack.setCustomSpacing(12, after: navigationLabel)
        ...
        NSLayoutConstraint.activate([
            ...
            // Navigation button height is determined by its internal content insets
        ])
    }
}

// Simple helper class for the section headers
...
// A simple struct to define an item in the segment
struct SegmentItem {
    let title: String
    let iconName: String
}
...
// The custom control
class GenericSegmentedTile: UIView {
    ...
    private let backgroundView: UIView = {
        let v = UIView()
        // Color matching image_2.png dark container
        v.backgroundColor = UIColor(red: 0.15, green: 0.15, blue: 0.16, alpha: 1.0) // Custom dark grey
    ...
    private let stackView: UIStackView = {
        ...
        stack.spacing = 2 // Tiny space between buttons so background shows through as separator
        stack.translatesAutoresizingMaskIntoConstraints = false
        return stack
    }()
    
    // MARK: - Init/Setup
    init(items: [SegmentItem], defaultIndex: Int = 0) {
        super.init(frame: .zero)
    ...
    private func createButtons(from items: [SegmentItem]) {
        for (index, item) in items.enumerated() {
            let btn = createButton(title: item.title, imageName: item.iconName)
            btn.tag = index // Tag used to identify which button was tapped
    ...
    private func updateSelectionState() {
        for (index, button) in buttons.enumerated() {
            if index == selectedIndex {
                // Selected State (Lighter Grey BG, Brighter Text)
                // Color matching selected state in image_2.png
                button.configuration?.baseBackgroundColor = UIColor(red: 0.25, green: 0.25, blue: 0.26, alpha: 1.0)
                button.configuration?.baseForegroundColor = .white
            } else {
                // Unselected State (Transparent BG, Dimmer Text)
