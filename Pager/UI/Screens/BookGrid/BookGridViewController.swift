...
class BookGridViewController: UIViewController, UICollectionViewDelegateFlowLayout, UISearchResultsUpdating, UITextFieldDelegate {
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { [weak self] _ in
            ...
            let wantToReadAction = UIAction(
                ...
                if isAlreadyInWantToRead {
                    ...
                    case .failure(let error):
                        if error == .bookAlreadyInCollection {
                            let alert = UIAlertController(title: "Already Added", message: "This book is already in the selected collection.", preferredStyle: .alert)
                            alert.addAction(UIAlertAction(title: "OK", style: .default))
                            self.present(alert, animated: true)
                        }
                        print("Error adding: \(error)")
            ...
            let addToCollectionMenu = UIMenu(title: "Add to Collection", image: UIImage(systemName: "folder.badge.plus"), children: collectionItems)
            var childCollection: [UIMenuElement] = []
            if viewModel.currentCollection?.name != DefaultsName.wantToRead {
                childCollection.append(wantToReadAction)
            }
            ...
            if let collection = self.viewModel.currentCollection {
                let deleteAction = UIAction(title: "Remove from Collection", image: UIImage(systemName: "minus.circle"), attributes: .destructive) { _ in
                    ...
                    case .success:
                        ...
                        Toast.show(message: "Removed from Collection", in: self.view)
                        self.refreshData()
                    case .failure(let error):
                        print("Error: \(error)")
                    }
    ...
    func refreshData() {
        self.viewModel.getUpdatedBookList()
        let currentSearch = searchController.searchBar.text ?? ""
        let _ = viewModel.searchBook(searchText: currentSearch)
        self.collectionView.reloadData()
        self.updateEmptyState()
    }
    ...
    func showAddItemAlert(book: Book) {
        let alertController = UIAlertController(title: "Add New Collection", message: "Enter the name for the new Collection.", preferredStyle: .alert)
    ...
    func showNameExistsAlert(name: String) {
        let alertController = UIAlertController(title: "Collection Already Exists!", message: "The Collection '\(name)' is already in your list.", preferredStyle: .alert)
