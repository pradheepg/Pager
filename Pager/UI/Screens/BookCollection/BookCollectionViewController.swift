...
class BookCollectionViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, ListViewControllerDelegate, UITextFieldDelegate, UISearchBarDelegate {
    ...
    private func loadData() {
        ...
        let sortedCollections = allCollections.sorted { (first, second) -> Bool in
        ...
        self.allItems = sortedCollections
    ...
    func filterContentForSearchText(_ searchText: String?) {
        ...
        self.items = self.allItems.filter { collection in
            ...
            return name.range(of: trimmedText, options: [.caseInsensitive, .diacriticInsensitive]) != nil
        }
...
//    }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        guard !items[indexPath.row].isDefault else { return nil }
    ...
    func showNameExistsAlert(name: String) {
        let alertController = UIAlertController(
            ...
            message: "The Collection '\(name)' is already in your list.",
            preferredStyle: .alert
        )
    ...
    private func deleteItem(at indexPath: IndexPath) {
        ...
        switch result {
        case .success:
            if let index = allItems.firstIndex(of: collectionToDelete) {
                allItems.remove(at: index)
            }
            
            items.remove(at: indexPath.row)
            
            Toast.show(message: "Collection deleted", in: self.view)
    ...
    func showEditAlert(at indexPath: IndexPath) {
        ...
        let alertController = UIAlertController(title: "Rename Collection", message: "Enter a new name.", preferredStyle: .alert)
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            ...
            guard newName != collection.name else { return }
            
            let nameExists = self.allItems.contains { item in
